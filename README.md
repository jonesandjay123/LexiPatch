...
這是一個 Android 應用程式，專為學習英文單字設計。它包含桌面小工具 (Widget)、單字管理 (Library)、JSON 匯入匯出、以及透過 AI 自動生成單字 (Paste) 的功能，並支援即時發音 (TTS)。
...
3.  **桌面小工具 (Widgets)**
    *   尺寸為 **2x1** (單層高度)。
    *   **全新設計**：深色質感背景 (Dark Mode)、圓角設計、質感圖示。
    *   **上一頁/下一頁 (Skip Prev/Next)**：即時切換單字。
    *   **發音 (Speak)**：點擊喇叭圖示 (Brand Awareness Icon) 可朗讀當前單字。
    *   **點擊文字**：可直接開啟 App。

4.  **匯入與匯出 (Import / Export) - NEW!**
    *   **匯出 (Export)**：支援將單字庫匯出為 JSON 檔案，方便備份或分享。
    *   **匯入 (Import)**：可讀取備份的 JSON 檔案並還原單字庫 (覆蓋模式)。
    *   入口位於 Library 頁面右上角選單。
